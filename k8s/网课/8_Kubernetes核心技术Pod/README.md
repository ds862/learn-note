...
可以通过以下命令，给节点新增标签，然后节点选择器就会进行调度了，如将节点设为dev
...
```
kubectl label node node1 --show-labels
```
...
节点亲和性 **nodeAffinity** 和 之前nodeSelector 基本一样的，根据节点上的 标签约束 来决定Pod调度到哪些节点上
...
（1）env_role 必须在 dev和 test 中 

支持常用操作符：in（在里面）、NotIn（不在里面）、Exists（存在）、Gt（大于）、Lt（小于）、DoesNotExists（不存在）

...
class RendezvousBackend(ABC):
    ...
    def get_state(self) -> Optional[Tuple[bytes, Token]]:
        log.debug(f" =====> 当前类名称：{self.__class__.__name__}")
        log.debug(f" =====> 当前函数名：{sys._getframe().f_code.co_name}")
        """Gets the rendezvous state.得到rdzv状态

        Returns:
              encoded的rdzv状态及其防御令牌的元组，如果在backend中没有找到状态，则为' None '。
    ...
    def set_state(
        ...
        log.debug(f" =====> 当前类名称：{self.__class__.__name__}")
        log.debug(f" =====> 当前函数名：{sys._getframe().f_code.co_name}")
        """Sets the rendezvous state. 设置rdzv状态
          新的rdzv状态的设置是有条件的
        The new rendezvous state is set conditionally:

              如果规定的token与存储在backend的fencing token匹配，则状态将被更新。
              新的状态将连同它的fencing token一起返回给调用者。
          - If the specified ``token`` matches the fencing token stored in the
            ...
            to the caller along with its fencing token.
              如果规定的token与存储在backend的fencing token不匹配，则状态不会被更新;
              起而代之的是将现有的状态及其fencing token返回给调用者。
          - If the specified ``token`` does not match the fencing token stored
            ...
            state along with its fencing token will be returned to the caller.
              如果规定的token为'None'，则只有在backend没有存在状态时才会设置新状态。
              新状态、或现有状及其fencing token将返回给调用者。
        ...
        Args:
            ...
            token:
                  一个可选的fencing token，通过之前调用的'get_state'或'' set_state() ''重新获取。
                An optional fencing token that was retrieved by a previous call
                to :py:meth:`get_state` or ``set_state()``.

        Returns:
              序列化rdzv状态的tuple，它的fencing token,和一个布尔值，指示设置尝试是否成功
...
class RendezvousTimeout:
    ...
    Args:
        ...
        close:
              在调用RendezvousHandler.set_closed`或`RendezvousHandler.shutdown`后，
              预计rdzv结束的时间(以秒为单位)。默认为30秒。
...
class _NodeDesc:
    ...
    """Describes a node in the rendezvous. 描述rdzvz中的节点
...
class _NodeDescGenerator:
    """Generates node descriptors. 生成节点描述符
     节点描述符是一个FQDN、一个进程id和一个自动递增的整数，唯一标识rdzv中的节点。
    ...
    def generate(self) -> _NodeDesc:
        log.debug(f" =====> 当前类名称：{self.__class__.__name__}")
        log.debug(f" =====> 当前函数名：{sys._getframe().f_code.co_name}")
        # 这个方法可以被多个线程并发调用;因此,我们必须以原子的方式递增整数。
...
class _RendezvousState:
    """Holds the state of a rendezvous. 保持RDZV的状态。

    Attributes:
        round:
            The current round of the rendezvous.  目前的rdzv轮次
        complete:
            A boolean value indicating whether the current round of the
            rendezvous is complete.  一个布尔值，指示当前一轮会合是否完成。
        deadline:
              如果当前这轮会合仍在等待节点加入，则认为会合已完成的时间。（最长等待时间）
...
class _RendezvousStateHolder(ABC):
    """Holds the shared rendezvous state synced with other nodes.保持与其他节点同步的共享rdzv状态"""
    ...
    def sync(self) -> Optional[bool]:
        log.debug(f" =====> 当前类名称：{self.__class__.__name__}")
        log.debug(f" =====> 当前函数名：{sys._getframe().f_code.co_name}")
        """Reads or writes the latest state.读写最新状态

        Returns:
              一个布尔值，指示本地状态(如果标记为dirty)是否与其他节点成功同步。
...
class _BackendRendezvousStateHolder(_RendezvousStateHolder):
    ...
    Args:
        ...
        cache_duration:
              在再次从backend请求最后一个rdzv状态之前,缓存该状态的时间量(以秒为单位)。
    ...
    def mark_dirty(self) -> None:
        ...
        log.debug(f" =====> 当前函数名：{sys._getframe().f_code.co_name}")
        """See base class.
          如果本地rdzv状态是dirty的，那么下一个同步调用将尝试将更改写回后端。
          但是，如果另一个具有相同状态的节点也在我们之前进行了更改并将其写入，那么这种尝试可能会失败。
...
class _RendezvousContext:
    """Holds the context of the rendezvous.保存rdzv的上下文信息。

    Attributes:
        node:
              与当前rdzv handler实例相关联的节点描述符
...
class _DistributedRendezvousOpExecutor(_RendezvousOpExecutor):
    """Executes rendezvous operations using a shared state.使用共享状态去执行rdzv操作
    ...
    Args:
        node:
            ...
            instance.
        state_holder:
              用于与其他节点同步rdzv状态的'rendezvous stateholder'。
    ...
    def run(
        ...
        while action != _Action.FINISH:
            # 读取或写入rdzv中所有节点共享的最新rdzv状态。
            # 请注意，如果另一个节点在我们之前同步了它的更改，那么我们的本地更改可能会被该节点覆盖。
            ...
            if has_set is not None:
                if has_set:
                    # {self._node}节点成功在rdzv'{self._settings.run_id}'中将其本地更改与其他节点同步。
                    log.debug(
    ...
    def _add_to_participants(self) -> None:
        # 添加节点到{self._state.round}(当前rdzv轮次)的participants中。等待同步。
        log.debug(f" =====> 当前类名称：{self.__class__.__name__}")
        log.debug(f" =====> 当前函数名：{sys._getframe().f_code.co_name}")
        ...
        # 节点rank暂时被设为0，一旦rdzv完成，参与者的rank将被设定。
...
def _should_keep_alive(ctx: _RendezvousContext) -> bool:
    ...
    log.debug(f" =====> 当前函数名：{sys._getframe().f_code.co_name}")
    """Determines whether a keep-alive heartbeat should be sent. 确定是否应该发送一个保持活动的心跳"""
    try:
...
class _RendezvousJoinOp:
    ...
    def __call__(self, ctx: _RendezvousContext, deadline: float) -> _Action:
        ...
        #如果我们是rdzv的一部分，而且rdzv已经完成，那就没有进一步的行动可采取了。
        ...
        if now > deadline:
            ...
            # 如果我们仍然有时间回退(在行动最后期限之上的短时间内)，试着离开集合点。
            # 如果我们不能也没关系，因为我们的生命最终会死亡。
        ...
        if state.complete:
            ...
            # 如果rdzv包括我们在内有足够人数的参与者，请检查我们是否已超过rdzv的最后期限。如果是，完成它。
...
class DynamicRendezvousHandler(RendezvousHandler):
    ...
    def next_rendezvous(self) -> Tuple[Store, int, int]:
        ...
        # 如果这是第一次运行，则将执行延迟一小段随机时间。这将略微倾斜节点之间的rdzv尝试，并减少后端上的负载。
...
def create_handler(
    ...
    parameters. 从指定的参数创建一个新的:py:class:`DynamicRendezvousHandler`

    Args:
        store:
            返回C10d store作为rdzv的一部分。
            The C10d store to return as part of the rendezvous.
        backend:
            使用backend用于保持rdzv状态
            The backend to use to hold the rendezvous state.
